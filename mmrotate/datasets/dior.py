...
class DIORDataset(CustomDataset):
    ...
    """
    # CLASSES = ('dam', 'stadium', 'bridge', 'Expressway-Service-area', 'Expressway-toll-station', 'chimney', 'trainstation', 'harbor', 
    #  'ship', 'groundtrackfield', 'airplane', 'windmill', 'tenniscourt', 'baseballfield', 'golffield', 'basketballcourt', 'vehicle',
    #  'storagetank', 'overpass', 'airport')

    # CLASSES = ('large-vehicle', 'swimming-pool', 'helicopter', 'bridge', 'plane', 'ship', 'soccer-ball-field', 'basketball-court', 'unknown')

    # CLASSES = ['airplane', 'airport', 'baseballfield', 'basketballcourt', 'bridge', 'chimney', 'Expressway-Service-area', 'Expressway-toll-station','dam', 'golffield', 'unknown']

    CLASSES = ['airplane', 'baseballfield', 'bridge', 'chimney','dam','Expressway-Service-area','Expressway-toll-station', 'golffield', 'harbor', 
    'overpass', 'ship', 'stadium', 'storagetank', 'tenniscourt', 'trainstation', 'vehicle', 'unknown'] #'airport','basketballcourt','groundtrackfield','windmill'  'unknown'   
    
    PALETTE = [(0, 220, 176), (92, 0, 73),
            (0, 0, 142), (0, 80, 100), (106, 0, 228), (0, 60, 100),
            (255, 99, 164), (0, 0, 70), (0, 0, 192), (250, 170, 30),
    ...
    def load_annotations(self, ann_file):
        ...
        """
        if self.test_all:
                self.CLASSES = ['airplane', 'baseballfield', 'bridge', 'chimney','dam','Expressway-Service-area','Expressway-toll-station', 'golffield', 'harbor', 
        ...
        for img_id in img_ids:
            ...
            for obj in root.findall('object'):
                ...
                poly = np.array([
                    float(obj.find('robndbox/x_left_top').text),
                    float(obj.find('robndbox/y_left_top').text),
                    float(obj.find('robndbox/x_right_top').text),
                    float(obj.find('robndbox/y_right_top').text),
                    float(obj.find('robndbox/x_right_bottom').text), 
                    float(obj.find('robndbox/y_right_bottom').text), 
                    float(obj.find('robndbox/x_left_bottom').text), 
                    float(obj.find('robndbox/y_left_bottom').text), 
                ],
                                dtype=np.float32)
        ...
        # print(f"ign_poly:{self.ign_poly}")
        return data_infos
