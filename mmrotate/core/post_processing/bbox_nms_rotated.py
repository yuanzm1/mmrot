...
def multiclass_nms_rotated(multi_bboxes,
    ...
    """
    #对所有的检测框整体进行nms，不区分类别
    # pdb.set_trace()
    max_scores, _ = torch.max(multi_scores[:, :-1], dim=1)
    _, max_keep = nms_rotated(multi_bboxes, max_scores, 0.5)
    multi_bboxes = multi_bboxes[max_keep]
    multi_scores = multi_scores[max_keep]
    # pdb.set_trace()
