...
def main():
    ...
    if not distributed:
        ...
        outputs = multi_gpu_test(model, data_loader, args.tmpdir,
                                 args.gpu_collect)
    # # 使用pickle模块保存字典到文件
    # with open("/mnt/disk2/yuanzm/weights/dota_153638_trans_features.pkl", 'wb') as file:
    #     pickle.dump(model.module.roi_head.cls_trans_feature, file)
    final_accs = torch.zeros_like(model.module.roi_head.accum_acc)
    for c in range(final_accs.shape[0]):
        if model.module.roi_head.accum_cnt[c] > 0:
            final_accs[c] = model.module.roi_head.accum_acc[c] / model.module.roi_head.accum_cnt[c]
        else:
            final_accs[c] = torch.nan  # 对于没有样本的类，标记为 NaN
    print()
    print("text feature alignment acc:",final_accs.detach().cpu().numpy())
